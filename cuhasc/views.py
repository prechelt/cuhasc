...
def _svg_grid(lines: list[str], language: str) -> dict[str, float]:
    """Append the axis, dimension lines and labels shared by both plots.
...
def _team_culture_profile_svg(team_profile: dict, language: str) -> str:
    ...
    rows = _svg_grid(lines, language)
    # a small dot per member on each Dimension line, plus an optional member label
...
def show_team(request, id, token):
    ...
    svg = mark_safe(_team_culture_profile_svg(team_profile, language)) if team_profile['members'] else None
